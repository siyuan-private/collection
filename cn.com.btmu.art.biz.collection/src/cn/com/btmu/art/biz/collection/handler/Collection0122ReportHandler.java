...
import cn.com.btmu.art.biz.collection.dataset.collection.impl.QOCollection0122Impl;
...
public class Collection0122ReportHandler extends AbstractComReportQueryHandler {
   ...
   */
  public void getCSV() {
    logger.debug("Collection0122ReportHandler-getCSV start");
    ...
    qm.setQueryObjectClass(QOCollection0122.class);
    ...
    //帐票ID
    csv.add(ComBizConstant.COMMON_REPORT_ID_ONE_HUNDRED_TWENTY_TWO);
    //帐票保存路径
    ...
    for (DataObject qo : resultList) {
      QOCollection0122Impl  dojb = (QOCollection0122Impl) qo;
      recNo = recNo + 1;
      if (recNo == 1
      ...
      // 业务种类
      csv.add(dojb.getBizType());
      // 寄单种类
      csv.add(dojb.getDispchDocType());
      // 快递号码
    ...
    logger.debug("Collection0122ReportHandler-getCSV end");
  }
