...
/** 
 * NoticeLetterNoticeLetterAjaxForDeleteDataBuilder  
 * 通知书打印删除节点用DataBuilder.
 * 
 * @author iVision
 * @version 1.0.0
 */
public class NoticeLetterNoticeLetterAjaxForDeleteDataBuilder implements
  ...
  public void buildInput(BbNoticeLettInfoVO vo,
    ...
    logger
        .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildInput start");
    // VO为空时返回，不进行后面的属性赋值操作
    if (vo == null) {

      return;
    }
    //当context中的通知书基本信息表为空时，实例化
    if (context.getBbNoticeLetterInfo() == null) {
      logger
          .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildInput context.getBbNoticeLetterInfo()为空");
      //设置context中的通知书基本信息表
      context.setBbNoticeLetterInfo(new BbNoticeLettInfoImpl());
    }

    //取得通知书基本信息
    BbNoticeLettInfo bbNoticeLettInfo = context.getBbNoticeLetterInfo();
    ...
    logger
        .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildInput end");
  ...
  public Object buildOutput(NoticeLetterNoticeLetterAjaxContext context,
    ...
    logger
        .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildOutput start");
    ...
    if (noticeLettInfo != null) {
      logger
          .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildOutput 复制属性");
      //将BO赋值到VO中
      ...
      vo.setNtid(noticeLettInfo.getId());
    ...
    logger
        .debug("NoticeLetterNoticeLetterAjaxForDeleteDataBuilder-buildOutput end");
    //返回VO
    return vo;
  }
