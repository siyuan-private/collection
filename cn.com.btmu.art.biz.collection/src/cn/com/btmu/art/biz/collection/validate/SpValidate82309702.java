...
/**
 * SpValidate82309702业务校验
 * 
 * @author ivision
 * @version 1.0.0
 */
public class SpValidate82309702 extends
		AbstractBizCheckUnit<CollectionCollChgbkDetailContext> {
	 ...
	 */
	@Override
	protected boolean canExecute(CollectionCollChgbkDetailContext data) {
		// 直接返回true，必须进行校验
		return true;
	}
	 ...
	 */
	@Override
	protected void doCheckLogic(CollectionCollChgbkDetailContext data,
			ErrorContainer errorContainer) {

		logger.debug("SpValidate82309702-doCheckLogic start");
		...
		if (feeRegDtl.size() > 0) {
			...
			String lableFee = i18nUtil.getI18nMessage(data.getSystemPara()
					.getLocale(), LabelComConstant.LABEL_COM_02068);

			logger
					.debug("SpValidate82309702-doCheckLogic 手续费不能通过特殊画面收取，只能选择手续费模块中的单独记账收取");
		...
		logger.debug("SpValidate82309702-doCheckLogic end");
	}
}
