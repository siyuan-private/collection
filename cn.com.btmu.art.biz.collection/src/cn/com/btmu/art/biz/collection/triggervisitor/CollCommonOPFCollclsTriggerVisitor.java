 ...
 * @version 1.0.0
 */
public class CollCommonOPFCollclsTriggerVisitor extends
    AbstractOPFCollClsTriggerVisitor<AbstractProtoDetailContext> {
  private Logger logger = LoggerFactory.getLogger(this.getClass());
  private static CollCommonOPFCollclsTriggerVisitor instance = new CollCommonOPFCollclsTriggerVisitor();

  private static DateUtil dateUtil = (DateUtil) FactoryBeanUtil
      .getBean(BizBeanNameConst.I00300_UTIL_DATE);

  private CollCommonOPFCollclsTriggerVisitor() {
  }

  public static CollCommonOPFCollclsTriggerVisitor getInstance() {
    return instance;
  }
  ...
  public List<OPFTriggerData> buildMediaData(
    ...
    if (bizCollection.getOpfData() == null
    		|| bizCollection.getOpfData().getOverdue() != null
    		|| bizCollection.getOpfData().getOverdue().size() == 0) {
    	return null;
    }
    
    // 创建自动触发对象
    List<OPFTriggerData> triggerDataLst = new ArrayList<OPFTriggerData>();
    for (CollHistInfo collHistInfo : bizCollection.getOpfData().getOverdue()) {
      // 新建单条自动触发对象
      ...
      //  业务主键
      triggerData.setTargetCommBizNo(collHistInfo.getSCrBizInfo()
          .getBizInfoId());
