 ...
 * 
 * 托收一览操作查询数据转换用DataBuilder.
 * 
 * @author ivision
 * @version 1.0.0
 */
public class CollectionQueryByOprDataBuilder extends
  ...
  public void buildInput(BillOfCollectionQueryVO vo,
    ...
    logger.debug("CollectionQueryByOprDataBuilder-buildInput start");
    if (vo == null) {
      logger.debug("VO为空校验");
      logger.debug("CollectionQueryByOprDataBuilder-buildInput end");
      return;
    }
    // 从context中取得QM
    ...
    // 设置查询模板的
    qm.setQueryObjectClass(QOCollectionOpr.class);
  ...
  public Object buildOutput(ComQueryContext context,
    ...
    logger.debug("CollectionQueryByOprDataBuilder-buildOutput start");
    // 从context取得查询结果
    QOCollectionOpr[] qoLst = (QOCollectionOpr[]) context.getQueryModelResult();
    if(qoLst == null){
    	return null;
    }
    // 新建变量.voLst
    ...
    logger.debug("CollectionQueryByOprDataBuilder-buildOutput end");
    // 返回变量.volst
    return voLst;
  }
