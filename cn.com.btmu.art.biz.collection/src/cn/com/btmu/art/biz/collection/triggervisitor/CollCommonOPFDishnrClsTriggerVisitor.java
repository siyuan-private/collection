 ...
 * @version 1.0.0
 */
public class CollCommonOPFDishnrClsTriggerVisitor extends
    AbstractOPFCollClsTriggerVisitor<AbstractProtoDetailContext> {
  private Logger logger = LoggerFactory.getLogger(this.getClass());
  private static CollCommonOPFDishnrClsTriggerVisitor instance = new CollCommonOPFDishnrClsTriggerVisitor();

  private static DateUtil dateUtil = (DateUtil) FactoryBeanUtil
      .getBean(BizBeanNameConst.I00300_UTIL_DATE);

  private CollCommonOPFDishnrClsTriggerVisitor() {
  }

  public static CollCommonOPFDishnrClsTriggerVisitor getInstance() {
    return instance;
  }
  ...
  public List<OPFTriggerData> buildMediaData(
    ...
    if (bizCollection.getOpfData() == null
    		|| bizCollection.getOpfData().getProtest() != null
    		|| bizCollection.getOpfData().getProtest().size() == 0) {
    	return null;
    }
    
    // 创建自动触发对象
    List<OPFTriggerData> triggerDataLst = new ArrayList<OPFTriggerData>();
    for (DishnrHistInfo dishnrHistInfo : bizCollection.getOpfData().getProtest()) {
      // 新建单条自动触发对象
      ...
      //  业务主键
      triggerData.setTargetCommBizNo(dishnrHistInfo.getSCrBizInfo()
          .getBizInfoId());
