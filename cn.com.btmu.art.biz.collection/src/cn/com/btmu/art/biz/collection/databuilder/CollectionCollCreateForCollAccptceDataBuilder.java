...
/**
 * CollectionCollCreateForCollAccptceDataBuilder
 * 
 * 托收新建节点数据（承兑操作时）转换用DataBuilder.
 * 
 * @author ivision
 * @version 1.0.0
 */
public class CollectionCollCreateForCollAccptceDataBuilder
    extends
    AbstractComComplexDetailDataBuilder<CollectionCollAccptceDetailContext, BcMntnOpDtlVO> {
  ...
  protected void registBuildInVisitor(BcMntnOpDtlVO vo,
    ...
    logger
        .debug("CollectionCollCreateForCollAccptceDataBuilder-registBuildInVisitor start");
    ...
    putBuildInVisitor(context.getDataRepository().getOperationDataCollection()
        ...
        .getInstance());
    logger
        .debug("CollectionCollCreateForCollAccptceDataBuilder-registBuildInVisitor end");
  }
  ...
  protected void registBuildOutFromBizVisitor(
      CollectionCollAccptceDetailContext context) {
    logger
        .debug("CollectionCollCreateForCollAccptceDataBuilder-registBuildOutFromBizVisitor start");
    ...
    if (null != listCollHistInfo && listCollHistInfo.size() > 0) {
      putBuildOutBizVisitor(listCollHistInfo.get(0), DishnrHistInfoVOBizBuildOutVisitor
          .getInstance());
    }
    ...
    if (null != listFaultHistInfo && listFaultHistInfo.size() > 0) {
      putBuildOutBizVisitor(listFaultHistInfo.get(0), FaultHistInfoVOBizBuildOutVisitor
          .getInstance());
    }
    ...
    logger
        .debug("CollectionCollCreateForCollAccptceDataBuilder-registBuildOutFromBizVisitor end");
  }
