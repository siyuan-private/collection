...
/**
 * SpValidate823019业务校验
 * 
 * @author ivision
 * @version 1.0.0
 */
public class SpValidate823019 extends
	...
	/**
	 * 当【画：入账.入两个账户】勾选且【画：入账.入账账户#2.币种】和【画：新建.汇票金额.币种】不一致时，显示报错信息，提示需要定价
	 * 
	 * @return void
	 * @param data
	 *            上下文数据
	...
	protected void doCheckLogic(CollectionCollBookingDetailContext context,
		...
		logger.debug("SpValidate823019-doCheckLogic start");
		...
		// 当【画：入账.入两个账户】勾选且【画：入账.入账账户#2.币种】和【画：新建.汇票金额.币种】不一致时
		if (DataTypeConstant.DATA_TYPE_YES.equals(bcBookOpDtl
				.getEnter2AcctFlg())
				&& !ArtStringUtil.equals(bcBookOpDtl.getBookAcctCcy2(), bcInfo
						.getBxCcy())) {
			// 获取国际化标签转换工具类
			...
			logger.debug("【画：入账.入两个账户】勾选且【画：入账.入账账户#2.币种】和【画：新建.汇票金额.币种】不一致");
			// 返回错误消息ID ART00131_E,{0}与{1}不一致，请进行定价操作.
			errorContainer.error(createErrMessage(
					CollectionConstant.MSG_ID_ART00131_E, lableBookAcctCcy2,
					lableBxCcy));

		}

		logger.debug("SpValidate823019-doCheckLogic end");
	}
}
