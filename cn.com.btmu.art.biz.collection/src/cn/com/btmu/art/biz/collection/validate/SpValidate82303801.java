...
/**
 * SpValidate82303801业务校验
 * 
 * @author ivision
 * @version 1.0.0
 */
public class SpValidate82303801 extends AbstractBizCheckUnit<DetailContext> {
	...
	protected boolean canExecute(DetailContext data) {
		// 当手续费不为空时返回true，否则返回true
		return data.getFeeInfo() == null || data.getFeeInfo().length == 0 ? false
				: true;
	}
	...
	protected void doCheckLogic(DetailContext data,
		...
		logger.debug("SpValidate82303801-doCheckLogic start");
		...
		// 当手续费大于5条时
		if (feeRegDtl != null && feeRegDtl.size() > 5) {
			// 获取国际化标签转变工具
			...
			logger.debug("SpValidate82303801-doCheckLogic 手续费的条数大于5条");
		...
		logger.debug("SpValidate82303801-doCheckLogic end");
	}
}
