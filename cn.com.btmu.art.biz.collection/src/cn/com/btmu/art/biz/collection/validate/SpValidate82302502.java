...
/**
 * SpValidate82302502业务校验
 * 
 * @author ivision
 * @version 1.0.0
 */
public class SpValidate82302502 extends
		AbstractBizCheckUnit<CollectionCollBookingDetailContext> {

	private Logger logger = LoggerFactory.getLogger(this.getClass());

	/**
	 * 当【画：入账.入账账户#2.币种】不为空时，【画：新建.汇票金额.币种】为CNY时.
	 * 
	 * @return boolean 判断结果
	 * @param data
	 *            数据存储
	...
	protected boolean canExecute(CollectionCollBookingDetailContext data) {
		...
		logger.debug("SpValidate82302502-canExecute start");
		...
		if (CollectionConstant.CCY_CNY.equals(bcInfo.getBxCcy())
			...
			logger.debug("SpValidate82302502-canExecute 【画：新建.汇票金额.币种】为CNY");
			logger.debug("SpValidate82302502-canExecute returnValue true");
			logger.debug("SpValidate82302502-canExecute end");
			// 返回true
			return true;
		}

		logger.debug("SpValidate82302502-canExecute 【画：新建.汇票金额.币种】不为CNY");
		logger.debug("SpValidate82302502-canExecute returnValue false");
		logger.debug("SpValidate82302502-canExecute end");
		// 当【画：入账.入账账户#2.币种】为空时，【画：新建.汇票金额.币种】不为CNY时
		return false;
	}

	/**
	 * 则【画：入账.入账账户#2.币种】必须输入CNY，否则报错
	 * 
	 * @return void
	 * @param data
	 *            上下文数据
	...
	protected void doCheckLogic(CollectionCollBookingDetailContext data,
		...
		logger.debug("SpValidate82302502-doCheckLogic start");

		// 取得托收入账操作明细获取操作数据
		BcBookOpDtl bcBookOpDtl = data.getDataRepository()
				.getOperationDataCollection().getBcBookOpDtl();
		// 【画：入账.入账账户#2.币种】未输入CNY
		if (!CollectionConstant.CCY_CNY.equals(bcBookOpDtl.getBookAcctCcy2())) {
			...
			// 转换国际化标签
			String lableBookAcctCcy2 = i18nUtil.getI18nMessage(data
					.getSystemPara().getLocale(),
					CollectionConstant.LABEL_823_00057);
			logger.debug("【画：入账.入账账户#2.币种】未输入CNY");
			// 返回错误消息ID ART00107_E， {0}的币种必须为CNY.
			errorContainer.error(createErrMessage(
					MsgIdComConstant.COM_MSG_ID_ART00107_E, lableBookAcctCcy2));

		}

		logger.debug("SpValidate82302502-doCheckLogic end");
	}
}
