...
$(function() {
	...
	$("#artform_def_detail").artForm({
				...
				save : function(target, esrc) {
					...
					$("#param_reqtype").val("Collection/CollModDoc/detail");
	...
	// 默认加载页面时缓存表单数据
	var detailInit = $.art.common
			.jsonStringInit(COMMON_CONSTANT.DETAIL_FORM_ID);

	// 记账用reqType初期化
	$("#genAccReqtype").val("Collection/CollModDoc/genacc");
	// 记账校验用用reqType初期化
	$("#genAccValidateReqtype").val("Collection/CollModDoc/genaccValidate");
	// reqtype初期化
	$("#commonReqtype").val("Collection/CollModDoc/detail");
	...
	$("#compareButton,#modHistButton,#dispch_add,#dispch_del").prop("disabled",
			false);
	...
	// 1.1.4）设置【画：新建.BPO】， 【画：新建.TSU】的可编辑
	...
	// 1.1.5）设置【画：新建.电报议付】，【画：新建.备注】的可编辑
	...
	// 1.1.7）设置【画：新建.Tenor.天数】，【画：新建.Tenor.日期】的可编辑
	...
	// 3- 当【画：新建.Tenor.种类】为‘O： others’时
	else if ("O" == tenortype) {
	...
	// 1.1.8）设置【画：信用证信息】板块和【画：非信用证信息】板块的初期显示
	// 1- 当【画：新建.信用证项下】为‘Y’
	...
	if ("Y" == ($(":radio[name='data/BcMntnOpDtlVO/lcFlg']:checked").val())) {
		...
		// b- 当【画：信用证信息.信用证编号】不为空时
			...
			commAjaxPost(jsonStr, function(expLcResult) {
				...
				$("[name='data/BcMntnOpDtlVO/tmpLcNo']").val($("#lcNo").val());
...
function generateValidate() {
	...
	var validateResult = $.art.Validator.validateBatch(
			$("[name='data/BbModChgbkDocInfoVO/regDate']").val(), "required",
			artI18N.Label.Label_COM_01186);
...
function submitValidate() {
	...
	// 2- 【画：新建.信用证项下】不得为空，否则报错（VALIDATE_00018）
	var lcFlg = $("[name = 'data/BcMntnOpDtlVO/lcFlg']:checked").val();
	if (lcFlg == undefined) {
		validateResult = $.art.Validator.validateBatch("", "required",
				artI18N.Label.Label_COM_02158);
		if (validateResult.state == false) {
			errMsg.push(validateResult.messages);
		}
	}
	// 3- 【画：新建.Tenor.种类】不得为空(‘请选择’)，否则报错（VALIDATE_00018）
	validateResult = $.art.Validator.validateBatch(
			$("[name = 'data/BcMntnOpDtlVO/tenorType']").val(), "required",
			artI18N.Label.Label_COM_00844);
	if (validateResult.state == false) {
		errMsg.push(validateResult.messages);
	}
	// 4- 【画：退改单.登记日】不得为空，否则报错（VALIDATE_00018）
	validateResult = $.art.Validator.validateBatch(
			$("[name = 'data/BbModChgbkDocInfoVO/regDate']").val(), "required",
			artI18N.Label.Label_COM_01186);
	if (validateResult.state == false) {
	...
	// 5- 参照【19(托收)_3.新建.xls】的 3-2.新建编辑_处理定义的 3.9）点击【画：保存提交】按钮时处理逻辑的
	// 3.9.1）数据校验逻辑
	...
	if (!SP_VALIDATE_823_017()) {
		return $.getExceptionMsg('ART00027-E',[artI18N.Label.Label_COM_01200]);
	}
	...
	var reimbBankCode = $("[name='data/BcMntnOpDtlVO/expLcMntnOpDtlVO/reimbBankCode']");// 【画：信用证信息.偿付行.行号】
	if (!SP_VALIDATE_823_021(reimbBankCode)) {
		return $.getExceptionMsg('ART00018-E',[artI18N.Label.Label_823_00030]);
	}
	// 38-
	// 当【画：新建.信用证项下】为‘Y’且【画：信用证信息.有偿付行】为‘Y’时，【画：信用证信息.偿付行.Swift】不得为空，否则报错（SP_VALIDATE_823_021）
	var reimbBankSwift = $("[name='data/BcMntnOpDtlVO/expLcMntnOpDtlVO/reimbBankSwift']");// 【画：信用证信息.偿付行.Swift】
	if (!SP_VALIDATE_823_021(reimbBankSwift)) {
		return $.getExceptionMsg('ART00018-E',[artI18N.Label.Label_823_00030]);
	}
	// 39-
	// 当【画：新建.信用证项下】为‘Y’且【画：信用证信息.有偿付行】为‘Y’时，【画：信用证信息.偿付行.名称】不得为空，否则报错（SP_VALIDATE_823_021）
	var reimbBankName = $("[name='data/BcMntnOpDtlVO/expLcMntnOpDtlVO/reimbBankName']");// 【画：信用证信息.偿付行.名称】
	if (!SP_VALIDATE_823_021(reimbBankName)) {
		return $.getExceptionMsg('ART00018-E',[artI18N.Label.Label_823_00030]);
	}
	// 40-
	// 当【画：新建.信用证项下】为‘Y’且【画：信用证信息.有偿付行】为‘Y’时，【画：信用证信息.偿付行.地址】不得为空，否则报错（SP_VALIDATE_823_021）
	var reimbBankAddr = $("[name='data/BcMntnOpDtlVO/expLcMntnOpDtlVO/reimbBankAddr']");// 【画：信用证信息.偿付行.地址】
	...
	// 48-
	// 【画：信用证信息.信用证金额.币种】不为空时，必须等于【画：新建.汇票金额.币种】，否则报错（VALIDATE_00035）：{0}的金额必须等于{1}。
	var lcAmtCcy = $("[name='data/BcMntnOpDtlVO/expLcMntnOpDtlVO/lcAmtCcy']")
			.val();
	...
	if (!SP_VALIDATE_823_004()) {
		return $.getExceptionMsg('ART82301-E');
	}
	...
	// LC信息.信用证金额#2-#5.金额】（VALIDATE_00034）
	$("#bbCombLcNo>table>tbody>tr:visible").each(function(i, o) {
		var thizTr = $(this);
		var index = i;
		var lcAmt = $(
				"[name='data/BcMntnOpDtlVO/bbCombLcMntnOpDtlVoList["
						+ index + "]/lcAmt']", thizTr);// 【画：信用证信息.Combined
		// LC信息.信用证金额.金额】
		var lcBalAmt = $(
				"[name='data/BcMntnOpDtlVO/bbCombLcMntnOpDtlVoList["
						+ index + "]/lcBalAmt']", thizTr);// 【画：信用证信息.Combined
		// LC信息.信用证余额.金额】
	...
	// 59 【画：信用证信息.转让金额.金额】不为空时，必须大于0，否则报错（VALIDATE_00031）
	...
	for (var m = 1; m <= ($("#dispchDocTable :checkbox").length - 2); m++) {
		if ($(":radio[name='data/buyBillBbMntnData/dispchDocInfoVOList[" + i
				+ "/dispchDocObj']:checked").val() == "Y") {
